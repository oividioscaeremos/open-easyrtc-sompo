...
//
// 0 if stillAlive is disabled (i.e. stillAliveInterval is 0)
// stillAliveInterval + stillAliveGracePeriod otherwise
//
var stillAliveTimeout;

eventListener.startStillAliveTimer = function(connectionObj, easyrtcid) {
   var appName = connectionObj.getAppName();

   if ( stillAliveTimeout === undefined ) {
       const interval = pub.getOption("stillAliveInterval");
       const grace    = pub.getOption("stillAliveGracePeriod");
       stillAliveTimeout = (interval ? interval + grace : 0);
   }
   if( stillAliveTimeout === 0 ) {
      return;
   }
